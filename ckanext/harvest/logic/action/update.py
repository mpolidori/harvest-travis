...
def get_mail_extra_vars(context, source_id, status):
    ...
    for harvest_object in harvest_objects:
        ...
        packages += '{}\n'.format(object_info.get('package'))
    ...
    for harvest_gather_error in islice(report.get('gather_errors'), 0, 20):
        errors += harvest_gather_error['message'] + '\n'

    if (obj_error != '' or job_error != ''):
        errors += toolkit._('Error Summary')
        errors += '\n'

    if (obj_error != ''):
        errors += toolkit._('Document Error')
        errors += '\n' + obj_error + '\n\n'

    if (job_error != ''):
        errors += toolkit._('Job Errors')
        errors += '\n' + job_error + '\n\n'
...
def prepare_summary_mail(context, source_id, status, template):
    extra_vars = get_mail_extra_vars(context, source_id, status)
    body = render_jinja2(template, extra_vars)
